...
func TestDBCourse_GetUserTermByStuId(t *testing.T) {
	...
	testCases := []testCase{
		{
			name:      "GetUserTermByStuId_Success",
			mockError: nil,
		...
		{
			name:           "GetUserTermByStuId_RecordNotFound",
			mockError:      nil,
		...
		{
			name:           "GetUserTermByStuId_DBError",
		...
		mockey.PatchConvey(tc.name, t, func() {
			...
			mockey.Mock((*gorm.DB).First).To(func(dest interface{}, conds ...interface{}) *gorm.DB {
				...
				case tc.mockRows != nil && errors.Is(tc.mockRows.Error, gorm.ErrRecordNotFound):
					...
					return mockGormDB
				default:
					userTerm, ok := dest.(*model.UserTerm)
					if ok {
						*userTerm = *tc.expectedResult
					}
					return mockGormDB
				}
			}).Build()

			result, err := mockDBCourse.GetUserTermByStuId(context.Background(), tc.stuId)

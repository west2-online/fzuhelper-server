...
func InitPaperRPC() (*paperservice.Client, error) {
	...
	r, err := etcd.NewEtcdResolver([]string{config.Etcd.Addr})
	if err != nil {
		return nil, fmt.Errorf("InitPaperRPC etcd.NewEtcdResolver failed: %w", err)
	}
	client, err := paperservice.NewClient(constants.PaperServiceName, client.WithResolver(r), client.WithMuxConnection(constants.MuxConnection))
	if err != nil {
		return nil, fmt.Errorf("InitPaperRPC NewClient failed: %w", err)
	}
	return &client, nil
}

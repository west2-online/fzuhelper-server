...
func main() {
	...
	if err != nil {
		log.Fatalf("Paper: new etcd registry failed, err: %v", err)
	}

	listenAddr, err := utils.GetAvailablePort()
	if err != nil {
		log.Fatalf("Paper: get available port failed, err: %v", err)
	}

	addr, err := net.ResolveTCPAddr("tcp", listenAddr)
	if err != nil {
		log.Fatalf("Paper: resolve tcp addr failed, err: %v", err)
	}

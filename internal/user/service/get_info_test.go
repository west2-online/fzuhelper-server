...
func TestUserService_GetUserInfo(t *testing.T) {
	...
	for _, tc := range testCases {
		mockey.PatchConvey(tc.name, t, func() {
			...
			mockey.Mock((*userDB.DBUser).UpdateStudent).To(
				func(ctx context.Context, userModel *dbmodel.Student) error {
					return nil
				},
			).Build()
...
func TestUserService_GetUserInfoYjsy(t *testing.T) {
	// 测试案例使用的结构体
	type testCase struct {
		name string
		// 原有字段
		expectedInfo      *dbmodel.Student
		expectedYjsy      *yjsy.StudentDetail
		expectedExist     bool
		mockError         error
		mockDBCreateError error
		mockYjsyError     error
	...
	for _, tc := range testCases {
		mockey.PatchConvey(tc.name, t, func() {
			...
			// Mock YJSY
			mockey.Mock((*yjsy.Student).WithLoginData).To(
				func(cookies []*http.Cookie) *yjsy.Student {
					return yjsy.NewStudent()
				},
			).Build()
			mockey.Mock((*yjsy.Student).GetStudentInfo).To(
				func() (*yjsy.StudentDetail, error) {
					return tc.expectedYjsy, tc.mockYjsyError
				},
			).Build()

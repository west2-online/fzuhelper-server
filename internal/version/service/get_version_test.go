...
func TestGetAlphaVersion(t *testing.T) {
	type testCase struct {
		name              string        // 测试用例名称
		mockJsonBytes     *[]byte       // mock返回的JSON数据
		mockError         error         // mock返回的错误
	...
	// 模拟数据
	mockVersion := &pack.Version{Url: "http://example.com/alpha.apk", Version: "1.0.0"}
	mockVersionBytes, _ := json.Marshal(mockVersion)
	...
	testCases := []testCase{
		...
		},
		{
			...
			expectedResult:    nil,
			expectingError:    true,
			expectedErrorInfo: "VersionService.GetAlphaVersion error:file not found",
		},
	}
	...
	for _, tc := range testCases {
		mockey.PatchConvey(tc.name, t, func() {
			...
			// 调用方法
			result, err := urlService.GetAlphaVersion()

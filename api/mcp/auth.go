...
func LoginTool() mcpgoserver.ServerTool {
	return mcpgoserver.ServerTool{
		Tool: mcp.NewTool("login",
			mcp.WithDescription("Use this tool when the user wants to log into the educational system. Call this when: user mentions logging in, needs to authenticate, or when other tools fail due to no active session. If JWCH_STUDENT_ID and JWCH_PASSWORD environment variables are set, this happens automatically on startup. Returns success message on successful login."),
...
func CheckSessionTool() mcpgoserver.ServerTool {
	return mcpgoserver.ServerTool{
		Tool: mcp.NewTool("check_session",
			mcp.WithDescription("Use this tool to verify if the current login session is still valid. Call this when: user asks about connection status, before performing operations after a long idle period, or to troubleshoot authentication issues. Returns session validity status."),
